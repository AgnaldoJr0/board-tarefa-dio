...
import com.dio.board_tarefa.dto.BoardColumnInfoDTO;
import com.dio.board_tarefa.exception.CardBlockedException;
import com.dio.board_tarefa.exception.CardFinishedException;
import com.dio.board_tarefa.exception.EntityNotFoundException;
import com.dio.board_tarefa.persistence.dao.BlockDAO;
import com.dio.board_tarefa.persistence.dao.CardDAO;
...
import com.dio.board_tarefa.persistence.entity.CardEntity;
...
public class CardService {
    ...
    public void cancel(final Long cardId, final Long cancelColumnId, final List<BoardColumnInfoDTO> boardColumnsInfo)
        ...
        try {
            ...
            if (dto.blocked()) {
                var message = "O card %s está bloquado, é necessário desbloquea-lo para mover".formatted(cardId);
                throw new CardBlockedException(message);
            }
            var currentColumn = boardColumnsInfo.stream()
                    .filter(bc -> bc.id().equals(dto.columnId()))
    ...
    public void block(final Long id, final String reason, final List<BoardColumnInfoDTO> boardColumnInfo)
            throws SQLException {
        try {
            var dao = new CardDAO(connection);
